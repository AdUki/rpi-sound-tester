...
// Encodes all channels().total() channels together as one Ogg/Opus logical stream (mapping family 255,
...
class OpusOggMultiEncoder {
 public:
  ...
  // Encode in_frames()*channels().total() interleaved float frames, appending any Ogg pages produced.
