...
constexpr size_t kKeepLines = 16;
...
std::vector<std::string> KmsgWatch::recent() const {
  std::lock_guard<std::mutex> lock(m_);
  return recent_;
}

void KmsgWatch::inject(const std::string& line) {
  if (!is_sync_error(line)) return;
  sync_errors_.fetch_add(1);
  std::lock_guard<std::mutex> lock(m_);
  recent_.push_back(line);
  if (recent_.size() > kKeepLines) recent_.erase(recent_.begin());
